...
class symbolicIconEngine : public QIconEngine {
   public:
    explicit symbolicIconEngine(QString file) : m_fileName(std::move(file)) {}

    ~symbolicIconEngine() override = default;

    symbolicIconEngine* clone() const override { return new symbolicIconEngine(m_fileName); }
    ...
    void paint(QPainter* painter, const QRect& rect, QIcon::Mode mode, QIcon::State state) override {
        ...
        const QString key =
            QStringLiteral("%1-%2x%3-%4")
                .arg(m_fileName, QString::number(rect.width()), QString::number(rect.height()), col.name());
