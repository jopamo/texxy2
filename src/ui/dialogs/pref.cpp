...
PrefDialog::PrefDialog(QWidget* parent) : QDialog(parent), ui(new Ui::PrefDialog) {
    ...
    if (syntaxNames.isEmpty()) {  // it's a shame that QObject::tr() doesn't work in Texxy::Config
            ...
            if (!win->ui->statusBar->isVisible()) {
                ...
                if (TabPage* tabPage = qobject_cast<TabPage*>(win->ui->tabWidget->currentWidget())) {
                    ...
                    for (int j = 0; j < win->ui->tabWidget->count(); ++j) {
                        ...
                        connect(thisTextEdit, &QPlainTextEdit::blockCountChanged, win,
                                &TexxyWindow::statusMsgWithLineCount);
                        connect(thisTextEdit, &TextEdit::selChanged, win, &TexxyWindow::statusMsg);
                        if (showCurPos)
                            connect(thisTextEdit, &QPlainTextEdit::cursorPositionChanged, win,
                                    &TexxyWindow::showCursorPos);
                    }
