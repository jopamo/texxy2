...
void TexxyWindow::removeTrailingSpacesIfNeeded(TextEdit* textEdit) {
    ...
    while (block.isValid()) {
        ...
        if (count > 0) {
            ...
            if (doubleSpace) {
                if (count != 2)
                    cursor.movePosition(QTextCursor::PreviousCharacter, QTextCursor::KeepAnchor, std::max(1, count - 2));
            }
            else if (singleSpace) {
                if (count > 1)
...
bool TexxyWindow::saveFile(bool keepSyntax,
    ...
    const QString elided =
        QStringLiteral("<p style='white-space:pre'>%1</p>").arg(fm.elidedText(tipDir, Qt::ElideMiddle,
                                                                              200 * fm.horizontalAdvance(QLatin1Char(' '))));
