...
static inline bool validate_utf8(std::string_view bytes) noexcept {
    ...
    while (p < end) {
        ...
        if (c >= 0xC2 && c <= 0xDF) {
            ...
            if ((p[0] & 0xC0) != 0x80) return false; // invalid continuation
            ++p;
            continue;
        }
        ...
        if (c >= 0xE0 && c <= 0xEF) {
            ...
            if ((c1 & 0xC0) != 0x80 || (c2 & 0xC0) != 0x80) return false; // invalid continuation

            // minimal checks
            if (c == 0xE0 && c1 < 0xA0) return false; // overlong
            if (c == 0xED && c1 >= 0xA0) return false; // surrogate half

            // extra surrogate guard
            const unsigned int code = ((c & 0x0F) << 12) | ((c1 & 0x3F) << 6) | (c2 & 0x3F);
            if (code >= 0xD800 && code <= 0xDFFF) return false; // surrogates invalid
            p += 2;
            continue;
        }
        ...
        if (c >= 0xF0 && c <= 0xF4) {
            ...
            if ((c1 & 0xC0) != 0x80 || (c2 & 0xC0) != 0x80 || (c3 & 0xC0) != 0x80) return false; // invalid continuation
